...
categorical_features = ['University Rating']
numerical_features = [
    'GRE Score','TOEFL Score','SOP','LOR','CGPA','Research'
]

important_features = [
    'CGPA','GRE Score','University Rating',
    'TOEFL Score','LOR','SOP','Research'
]
...
def generate_counterfactuals(query, nun):
    ...
    for i in range(len(important_features)):
        ...
        if pred != y0:
            key = tuple(round(base[f],8) for f in feature_order)
            if key not in seen:
                seen.add(key)
        ...
        for f in important_features[:i+1]:
            ...
            for _ in range(PERTURB_STEPS):
                ...
                if pred2 != y0:
                    key = tuple(round(perturbed[k],8) for k in feature_order)
                    if key not in seen:
                        seen.add(key)
...
print("\nScanning dataset for SPICE trade-offs")
print("="*80)

tradeoff_results = []

test_queries = test_df[test_df['target'] == 0]
...
for idx in range(len(test_queries)):
    ...
    # Require at least one feature difference + proximity change
...
if len(tradeoff_results) > 0:
    ...
    print("\nBEST TRADE-OFF INSTANCE (USE THIS IN PAPER)")
    print("="*80)
