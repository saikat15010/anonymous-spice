...
Uses shap.TreeExplainer for the XGBClassifier model.
"""
...
def main():
    ...
    # XGBoost binary often returns single 2D array of class 1 contributions
    if isinstance(shap_vals, list):
        ...
        if shap_vals.ndim == 3:
            ...
            # XGB binary: same magnitude for both classes (opposite signs)
    ...
    print(f"SHAP class 1 weights:")
    for f, w in zip(FEATURE_COLUMNS, shap_per_class[1]):
        print(f"  {f:<45} {w:.4f}")
    print()
    ...
    print("=" * 70)
    print("PertCF Table 5 Results - GMC (100 queries)")
    print("=" * 70)
    ...
    df.to_csv('table5_pertcf_GMC.csv', index=False)
    print(f"\nPer-query results saved to: table5_pertcf_GMC.csv")
