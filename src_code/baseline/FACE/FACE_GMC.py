...
# Subsample training data for FACE graph (full 100K+ is intractable)
...
def main():
    ...
    # Subsample training set for FACE graph (full 100K+ is intractable)
    if len(X_train_scaled) > TRAIN_SUBSAMPLE:
        sub_idx = rng.choice(len(X_train_scaled),
                             size=TRAIN_SUBSAMPLE, replace=False)
        X_face_train = X_train_scaled[sub_idx]
        print(f"Subsampling FACE training set: "
              f"{len(X_train_scaled)} -> {TRAIN_SUBSAMPLE}")
    else:
        X_face_train = X_train_scaled

    print(f"Dataset size: {len(X)}")
    print(f"FACE training graph size: {len(X_face_train)}")
    print(f"Drawing {n_take} queries (seed={RANDOM_SEED})\n")

    face = FACE(data=X_face_train, model=clf,
    ...
    print("NOTE: This may take a long time even with subsampling.\n")
    ...
    print("=" * 70)
    print("FACE Table 5 Results - GMC (100 queries)")
    print("=" * 70)
    ...
    df.to_csv('table5_face_GMC.csv', index=False)
    print(f"\nPer-query results saved to: table5_face_GMC.csv")
