"""
UFCE Baseline Evaluation — GMC Dataset
Suffian et al. (2024). User Feedback-Based Counterfactual Explanations.

NOTE: GMC has ~150K rows. The feasible-region pool (model-predicted
positives) can be very large. To keep k-NN tractable we subsample the
positive pool to FEASIBLE_POOL_SUBSAMPLE. This is disclosed in the script.
"""
...
# Cap the feasible region pool for tractability on large GMC
FEASIBLE_POOL_SUBSAMPLE = 5000
...
def main():
    ...
    pos_pool_raw    = X[pos_mask]
    pos_pool_scaled = all_X_sc[pos_mask]
    neg_pool_raw    = X[neg_mask]

    if len(pos_pool_raw) == 0:
        print("ERROR: no model-predicted positives.")
        return

    # Subsample feasible region pool for tractability on large GMC
    rng_sub = np.random.default_rng(RANDOM_SEED)
    if len(pos_pool_raw) > FEASIBLE_POOL_SUBSAMPLE:
        sub_idx = rng_sub.choice(len(pos_pool_raw),
                                 size=FEASIBLE_POOL_SUBSAMPLE, replace=False)
        pos_pool_raw    = pos_pool_raw[sub_idx]
        pos_pool_scaled = pos_pool_scaled[sub_idx]
        print(f"Subsampled feasible region pool: -> {FEASIBLE_POOL_SUBSAMPLE}")
    ...
    print("=" * 70)
    print("UFCE Results — GMC (100 queries)")
    ...
    df.to_csv('table5_ufce_GMC.csv', index=False)
    print(f"\nPer-query results saved to: table5_ufce_GMC.csv")
