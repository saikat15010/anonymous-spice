"""
UFCE Baseline Evaluation — Graduate Admission Dataset
======================================================
UFCE: User Feedback-Based Counterfactual Explanations
Suffian et al. (2024). International Journal of Computational Intelligence Systems.

FIX vs original draft:
  Best Sparsity / Best Proximity are now reported as the per-metric min
  across the candidate set (matching SPICE's reporting protocol), rather
  than the metric values of a single CF selected by a combined objective.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Metric helpers (identical to SPICE)
# ─────────────────────────────────────────────────────────────────────────────
...
def load_everything():
    ...
    X_all = combined[FEATURE_COLUMNS].values.astype(float)
    ...
    ranges = build_ranges(combined)

    return (combined, model, scaler, isf,
            cat_idx, num_idx, immutable_idx, mutable_idx, ranges)
...
def main():
    ...
    (combined, model, scaler, isf,
     cat_idx, num_idx, immutable_idx,
     mutable_idx, ranges) = load_everything()
    ...
    print(f"k-NN feasible region index built  (k={k_actual}, "
          f"pool size={len(pos_pool_raw)})")
    ...
    print(f"\nRunning UFCE on {n_take} queries "
          f"(k={k_actual}, {len(ALPHA_STEPS)} alpha steps)...\n")
    ...
    for qi in range(n_take):
        ...
        cands = ufce_generate(
            q_raw, feasible_neighbors, model, scaler,
            cat_idx, num_idx, mutable_idx, ranges)
    ...
    if len(df) > 0:
        print(f"Avg candidates per query   : "
              f"{df['n_candidates'].mean():.1f}\n")

    if len(df) == 0:
        print("ERROR: UFCE produced no valid counterfactuals.")
        return

    print("=" * 70)
    print("UFCE Results — Graduate Admission (k=5, 10 alpha steps, 100 queries)")
    ...
    df.to_csv('table5_ufce_GraduateAdmission.csv', index=False)
    print(f"\nPer-query results saved to: table5_ufce_GraduateAdmission.csv")
