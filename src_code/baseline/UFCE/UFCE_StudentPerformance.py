"""
UFCE Baseline Evaluation — Student Performance Dataset
Suffian et al. (2024). User Feedback-Based Counterfactual Explanations.

Protocol mirrors SPICE: same 100 queries (seed=42), same model,
same metric formulas, mean +/- std reporting.
"""
...
def main():
    ...
    print("=" * 70)
    print("UFCE Results — Student Performance (100 queries)")
    ...
    df.to_csv('table5_ufce_StudentPerformance.csv', index=False)
    print(f"\nPer-query results saved to: table5_ufce_StudentPerformance.csv")
