...
data = pd.read_csv('/Users/saikat/Desktop/Summer 2025/CFNet_AAAI_26/GMC Dataset/preprocessed_credit_data.csv')

# Check for missing values in target variable
print(f"Missing values in target variable: {data['SeriousDlqin2yrs'].isnull().sum()}")
print(f"Unique values in target: {data['SeriousDlqin2yrs'].unique()}")

# Remove rows with missing target values
print(f"Original dataset shape: {data.shape}")
combined_df_clean = data.dropna(subset=['SeriousDlqin2yrs'])
...
rf_model = XGBClassifier(n_estimators=100, random_state=0)
...
query_instance = {'RevolvingUtilizationOfUnsecuredLines': 0.189169052, 'age': 57.0, 'NumberOfTime30-59DaysPastDueNotWorse': 0.0, 'DebtRatio': 0.606290901, 'MonthlyIncome': 23684.0, 'NumberOfOpenCreditLinesAndLoans': 9.0, 'NumberOfTimes90DaysLate': 0.0, 'NumberRealEstateLoansOrLines': 4.0, 'NumberOfTime60-89DaysPastDueNotWorse': 0.0, 'NumberOfDependents': 2.0}

nun_instance   = {'RevolvingUtilizationOfUnsecuredLines': 0.162909136, 'age': 55.0, 'NumberOfTime30-59DaysPastDueNotWorse': 0.0, 'DebtRatio': 2.323870452, 'MonthlyIncome': 2500.0, 'NumberOfOpenCreditLinesAndLoans': 10.0, 'NumberOfTimes90DaysLate': 0.0, 'NumberRealEstateLoansOrLines': 4.0, 'NumberOfTime60-89DaysPastDueNotWorse': 0.0, 'NumberOfDependents': 2.0}
...
important_features = [f for f in ['RevolvingUtilizationOfUnsecuredLines', 'age', 'NumberOfTime30-59DaysPastDueNotWorse', 'DebtRatio', 'MonthlyIncome', 'NumberOfTimes90DaysLate', 'NumberOfOpenCreditLinesAndLoans', 'NumberOfTime60-89DaysPastDueNotWorse', 'NumberRealEstateLoansOrLines', 'NumberOfDependents'] if f not in immutable_features]


# Define categorical and numerical features
categorical_features = [f for f in ['NumberOfDependents', 'NumberOfTime30-59DaysPastDueNotWorse', 'NumberRealEstateLoansOrLines'] if f not in immutable_features]
numerical_features = [f for f in ['RevolvingUtilizationOfUnsecuredLines', 'age', 'DebtRatio', 'MonthlyIncome', 'NumberOfOpenCreditLinesAndLoans', 'NumberOfTimes90DaysLate', 'NumberOfTime60-89DaysPastDueNotWorse'] if f not in immutable_features]
...
feature_order = ['RevolvingUtilizationOfUnsecuredLines', 'age', 'NumberOfTime30-59DaysPastDueNotWorse', 'DebtRatio', 'MonthlyIncome', 'NumberOfOpenCreditLinesAndLoans', 'NumberOfTimes90DaysLate', 'NumberRealEstateLoansOrLines', 'NumberOfTime60-89DaysPastDueNotWorse', 'NumberOfDependents']
...
def find_best_candidate(counterfactual_candidates, query_instance, feature_order, lambda_value=0):
