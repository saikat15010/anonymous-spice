...
data = data.dropna(subset=['SeriousDlqin2yrs'])
print(f"Dataset shape after removing missing targets: {data.shape}")


# Get feature names
feature_names = data.drop(columns=['SeriousDlqin2yrs', 'ID']).columns.tolist()

# Define features and target
X = data.drop(columns=['SeriousDlqin2yrs', 'ID']).values
...
model_path = '/Users/saikat/Desktop/Summer 2025/CFNet_AAAI_26/GMC Dataset/best_credit_xgb_model.pkl'
...
all_feature_scores = []
sample_count = 0
max_samples = min(2000, len(X_test_scaled))  # Analyze up to 200 samples for efficiency
