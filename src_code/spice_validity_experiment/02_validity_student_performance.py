...
if __name__ == "__main__":
    data1 = pd.read_csv(BASE + "/Student Performance Datastet/AISTATS Student Performance/student_train.csv")
    data2 = pd.read_csv(BASE + "/Student Performance Datastet/AISTATS Student Performance/student_test.csv")
    data = pd.concat([data1, data2], ignore_index=True)

    for f in ["famsup", "higher", "internet", "romantic"]:
        if data[f].dtype == object:
            data[f] = data[f].map({"yes": 1, "no": 0})

    feature_order = [
        "age", "Medu", "Fedu", "studytime", "famsup", "higher", "internet", "romantic",
    ...
    semantic_weights = np.array([
        4.4797, 49.0592, 27.6233, 28.7447,
        0.3854, 66.1713, 16.8917, 2.7596,
        7.3206, 4.9569, 2.3510, 13.4245,
        650.8234, 720.1519
    ])
