...
if __name__ == "__main__":
    data = pd.read_csv(BASE + "/GMC Dataset/AISTATS GMC Dataset/preprocessed_credit_data.csv")
    data = data.dropna(subset=["SeriousDlqin2yrs"]).copy()
    ...
    model = XGBClassifier(n_estimators=50, random_state=0, eval_metric="logloss")
    model.fit(X_train_scaled, y_train)

    semantic_weights = np.array([
        0.2895, 1722.9974, 10328.1365, 10.9695,
        44.0967, 103.5971, 13770.3610, 12.5579,
        9505.8989, 256.6856
    ])
