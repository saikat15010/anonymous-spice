...
def main():
    (combined, model, scaler, isf, cat_idx, num_idx,
     immutable_idx, ranges) = load_everything()

    # NUN pool and query pool filtered by MODEL PREDICTION
    ...
    print("=" * 70)
    print("SPICE Table 5 Results - Graduate Admission (LIME, 100 queries)")
    ...
    df.to_csv('table5_spice_GraduateAdmission.csv', index=False)
    print(f"\nPer-query results saved to: table5_spice_GraduateAdmission.csv")
